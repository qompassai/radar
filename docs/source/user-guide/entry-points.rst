...
Radar derives its modularity in part by establishing a convention for how to compose new entry points.
...
.. note::
   ...
   This particular usage for the term *entry point* `originates with the MLFlow library <https://mlflow.org/docs/latest/projects.html#overview>`_, and we have adopted it for this project since Radar uses MLFlow_ on the backend to provide job tracking capabilities.
...
.. code-block:: python
   ...
   if __name__ == "__main__":
       log_level = os.getenv("RADAR_JOB_LOG_LEVEL", default="INFO")
       as_json = True if os.getenv("RADAR_JOB_LOG_AS_JSON") else False
...
Prefect is a modern workflow library that is aimed at helping data scientists set up task execution graphs with minimal changes to their existing code, and in Radar it provides a framework for wiring :doc:`task plugins <task-plugins>` together.
...
.. code-block:: python
   ...
   _PLUGINS_IMPORT_PATH: str = "radar_builtins"
