...
The generic functions (*generics* for short) in :py:mod:`radar.sdk.generics` provide standardized interfaces for sets of related methods.
...
This system means that **all** the underlying generic implementations, even the ones that come bundled with the Testbed, are plugins that are independent of the core :py:mod:`radar.sdk` package and users are free to create custom implementations and share them as Python packages.
...
=============================  =======================================
:py:func:`.fit_estimator`      `radar.generics.fit_estimator`
:py:func:`.estimator_predict`  `radar.generics.estimator_predict`
=============================  =======================================
...
Consider the following example for the :py:func:`~radar.sdk.generics.fit_estimator` generic, which has the following signature:
...
.. code-block:: python
   ...
   from radar.sdk.generics import fit_estimator
...
#. The first of the two implementations omits the `y` argument, which allows :py:func:`~radar.sdk.generics.fit_estimator` to be called without having to specify `y`.
...
Knowing this, we add the following to our `setup.cfg` file to register the implementation to the advertised plugin entry point for the :py:class:`~radar.sdk.generics.fit_estimator` generic:

.. code-block:: ini

   [options.entry_points]
   radar.generics.fit_estimator = tf_keras_model=fit_estimator_tf_keras.tf_keras_model
...
Once you have this configured for your package, all you need to do is ``pip install`` it and the new implementation will be available to the :py:class:`~radar.sdk.generics.fit_estimator` generic for dispatching the next time you use it.
...
.. tip::
   ...
   Readers are also encouraged to examine the source files of the Radar repository itself.
