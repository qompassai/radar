...
Radar contains a number of built-in task plugins that can be executed across all available examples.
...
In general while built-in task plugins are located in the ``task-plugins/radar_builtins`` folder, local tasks can be stored in the same way as local python functions in each example's src code folder.
