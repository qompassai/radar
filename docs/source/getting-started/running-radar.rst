.. This Software (Radar) is being made available as a public service by the
...
.. _getting-started-running-radar:
...
The Radar GitHub repository provides a `Cookiecutter <https://cookiecutter.readthedocs.io/en/latest/>`__ template in the ``cookiecutter-templates/cookiecutter-radar-deployment/`` folder that you can use to generate the scripts, configuration files, and Docker Compose files needed to run Radar on a single machine.
...
- :ref:`Working builds of the Radar container images <getting-started-building-the-containers>`

Quickstart
----------

The minimal terminal commands needed to configure and run a fresh installation of Radar are provided below.
This will generate a setup that is appropriate for testing Radar on your personal computer or laptop.
...
.. code:: sh
   ...
   cruft create https://github.com/usnistgov/radar --checkout $RADAR_BRANCH \
     --directory cookiecutter-templates/cookiecutter-radar-deployment

Cruft will now run and prompt you to configure the deployment. See the :ref:`Applying the template <getting-started-running-radar-applying-the-template>` section for detailed description of each prompt.

We recommend identifying a location to store datasets you will want to use with Radar at this point and setting the ``datasets_directory`` variable accordingly. See the :ref:`Downloading the datasets <getting-started-acquiring-datasets>` section for more details.
...
When you are done using Radar, navigate back to the configuration folder ``/path/to/deployments/folder/radar-deployment`` in your terminal and run,
...
The rest of this page is a detailed walk-through of the above commands, which includes information about more advanced topics, such as :ref:`enabling SSL/TLS for the NGINX and postgres services <running-radar-enabling-ssl-tls-in-nginx-and-postgres>` and :ref:`how to mount additional folders in the worker containers <running-radar-mounting-folders-in-the-worker-containers>`.

.. _getting-started-running-radar-applying-the-template:
...
Next, run cruft and apply the Radar Deployment template.
...
- **deployment_name:** A name to associate with the deployment.
  ...
  (default: ``Radar deployment``)
- **container_registry:** This is only needed if you are building and pushing Radar container images to your own Docker registry.
...
- **container_tag:** This should match the tags of the Radar container images you will be using (if you are using Docker, you can see the available tags by running ``docker images``).
...
- **systemd_required_mounts:** This applies only if you are deploying Radar on a Linux distribution that uses systemd, such as Ubuntu, Fedora, or Red Hat Enterprise Linux, otherwise leave this blank.
  The "required mounts" would be the storage devices mounted on the host machine, which may include NFS volumes or block storage volumes, that must be accessible before the Radar application containers can start.
  This list should include all the devices that store the container images, runtime files, and volume data for Radar.
  ...
  (default: *empty string*)
- **nginx_server_name:** If you are hosting Radar on an remote machine, such as a virtual private server, then this will either be a domain name like ``radar.example.org`` or the IP address you use to access the machine.
  If you are running Radar on your personal computer or laptop, then answer ``_``.
  (default: ``radar.example.org``)
- **nginx_expose_ports_on_localhost_only:** Choosing ``True`` will generate a ``docker-compose.yml`` file that binds the NGINX ports to the loopback interface (localhost, 127.0.0.1).
  ...
  Radar currently does not have an access control system, so for now it is recommended that you choose ``True`` on remote machines and use SSH or firewall rules to manage access.
  If you are running Radar on your personal computer or laptop, also choose ``True``.
  (default: ``True``)
...
- **pgadmin_default_email:** This email address will be used to create the default account on the `pgadmin4 app <https://www.pgadmin.org/docs/pgadmin4/latest/index.html>`__.
  (default: ``radar@example.com``)
- **num_tensorflow_cpu_workers:** Sets the number of Tensorflow CPU workers to use in your Radar deployment.
  Each worker is dedicated to watching a named queue and running one job at a time.
  (default: ``1``)
- **num_tensorflow_gpu_workers:** Sets the number of Tensorflow GPU workers to use in your Radar deployment.
  ...
  (default: ``0``)
- **num_pytorch_cpu_workers:** Sets the number of PyTorch CPU workers to use in your Radar deployment.
  Each worker is dedicated to watching a named queue and running one job at a time.
  (default: ``1``)
- **num_pytorch_gpu_workers:** Sets the number of PyTorch GPU workers to use in your Radar deployment.
  Each worker is assigned a dedicated GPU, watches a named queue, and runs one job at a time.
  The combined number of Tensorflow and PyTorch GPU workers cannot be greater than the number of GPUs available on the host machine.
  (default: ``0``)
- **datasets_directory:** Sets the datasets directory to be mounted into all worker containers in your Radar deployment.
  The provided directory will be mounted to `/radar/data` in each worker container.
...
.. margin::
   ...
   .. note::
      ...
      The *radar-deployment* prefix prepended to the files in the ``envs/`` and ``secrets/`` folders assumes the default value for the **deployment_name** variable.
...
.. code:: text
   ...
   │   ├── radar-deployment-db.env                            <- Sets environment variables that customize the Postgres database. Safe to commit to a git repo.
   │   ├── radar-deployment-dbadmin.env                       <- Sets environment variables that customize the pgadmin4 dashboard. Safe to commit to a git repo.
   │   ├── radar-deployment-mlflow-tracking.env               <- Sets environment variables that customize the MLflow Tracking service. Safe to commit to a git repo.
   │   ├── radar-deployment-restapi.env                       <- Sets environment variables that customize the Radar REST API service. Safe to commit to a git repo.
   │   ├── radar-deployment-worker-cpu.env                    <- Sets environment variables that customize the CPU-based Radar workers. Safe to commit to a git repo.
   │   └── radar-deployment-worker.env                        <- Sets environment variables that customize the Radar workers. Safe to commit to a git repo.
   ├── scripts
   │   ├── templates
   ...
   │   │   ├── minio-accounts.env.j2                            <- A Jinja2 template used to generate the secrets/radar-deployment-minio-accounts.env file.
   ...
   ├── secrets
   │   ├── radar-deployment-minio-accounts.env                <- Secrets file containing sensitive passwords generated by the scripts/generate_password_templates.py script. NOT SAFE TO SHARE OR COMMIT TO A GIT REPO.
...
.. _running-radar-enabling-ssl-tls-in-nginx-and-postgres:
...
Open the file in a text editor, find the block for the NGINX service (the block starts with ``radar-deployment-nginx:`` if you used the default value for **deployment_name**), and edit its ports and health check test to match the appropriate YAML snippet below.
...
.. _running-radar-mounting-folders-in-the-worker-containers:
...
Append the line ``- "/home/data:/radar/data:ro"`` to the ``volumes:`` subsections.
...
.. code:: yaml
     ...
     custom-container:
       ...
       command:
         - --wait-for
         - radar-deployment-redis:6379
         - --wait-for
         - radar-deployment-minio:9001
         - --wait-for
         - radar-deployment-db:5432
         - --wait-for
         - radar-deployment-mlflow-tracking:5000
         - --wait-for
         - radar-deployment-restapi:5000
         - tensorflow_cpu
       env_file:
...
.. _getting-started-running-radar-init-deployment:
...
In addition, if you wish to sync the plugins from a different Radar GitHub branch, such as the main branch, use the ``--branch <arg>`` option.
...
.. note::
   ...
   This will only work if you are deploying Radar on a Linux distribution that uses systemd, such as Ubuntu, Fedora, or Red Hat Enterprise Linux.
