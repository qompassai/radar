...
from radar.restapi.utils import setup_injection

from .__version__ import __version__ as RADAR_VERSION
from .db import db
...
def create_app(env: Optional[str] = None, injector: Optional[Injector] = None) -> Flask:
    """Creates and configures a fresh instance of the Radar REST API.
    ...
    if env is None:
        env = os.getenv("RADAR_RESTAPI_ENV", "test")
    ...
    api: Api = Api(
        ...
        doc=app.config["RADAR_SWAGGER_PATH"],
        url_scheme=app.config["RADAR_BASE_URL"],
    )
    ...
    if env != "prod":
        cors.init_app(
            app, resources={r"/api/*": {"origins": app.config["RADAR_CORS_ORIGIN"]}}
        )
