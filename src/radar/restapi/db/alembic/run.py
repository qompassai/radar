...
"""Command-line script for applying migrations to the Radar database.

The database connection details are set using environment variables, see
radar/restapi/config.py for more information.
...
Examples:
    ...
    To downgrade the database to a previous revision::
        ...
        radar-db downgrade --revision 6b432d0a5ac7
    ...
    To generate a new migration script::
        ...
        radar-db migrate --message "Add new table"
"""
...
def find_alembic_dir() -> Path:
    ...
    """
    module_spec = find_spec("radar.restapi.db.alembic")

    if module_spec is None or module_spec.submodule_search_locations is None:
        raise ModuleNotFoundError(
            "Unable to find the spec for the radar.restapi.db.alembic submodule. "
            "The radar package does not appear to be installed correctly."
        )
...
def is_db_uri_set() -> bool:
    ...
    """
    app_env = os.getenv("RADAR_RESTAPI_ENV", "dev")
    env_vars = {
        "dev": "RADAR_RESTAPI_DEV_DATABASE_URI",
        "prod": "RADAR_RESTAPI_DATABASE_URI",
    }

    if (env_var := env_vars.get(app_env)) is None:
        raise ValueError(
            f"Invalid value for RADAR_RESTAPI_ENV: {app_env}. "
...
def cli():
    """Command-line interface for managing the Radar database.
    ...
    \b
    RADAR_RESTAPI_ENV:
        The environment in which the Radar REST API is running. Valid values
        are 'dev' and 'prod'. Defaults to 'dev'.

    \b
    RADAR_RESTAPI_DEV_DATABASE_URI:
        The connection URI for the development database. Used when
        RADAR_RESTAPI_ENV is set to 'dev'.

    \b
    RADAR_RESTAPI_DATABASE_URI:
        The connection URI for the production database. Used when
        RADAR_RESTAPI_ENV is set to 'prod'.
    """
    pass
...
def autoupgrade() -> None:
    """Automatically upgrade the Radar database schema to the latest version."""
...
def upgrade(revision: str):
    """Upgrade the Radar database schema to the specified revision."""
    if not is_db_uri_set():
        return None
...
def downgrade(revision: str):
    """Downgrade the Radar database schema to an earlier version.
...
def history(indicate_current: bool):
    """Display the Radar database schema history."""
    if not is_db_uri_set():
        return None
...
@click.option(
    ...
    default=Path("src", "radar", "restapi", "db", "alembic"),
    help='Migration script directory (default is "src/radar/restapi/db/alembic")',
)
