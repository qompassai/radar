...
from radar.restapi.routes import (
    V1_AUTH_ROUTE,
...
from radar.sdk.utilities.contexts import sys_path_dirs
from radar.sdk.utilities.paths import set_cwd

LOGGER: BoundLogger = structlog.stdlib.get_logger()

ENV_RADAR_API: Final[str] = "RADAR_API"
ENV_RADAR_WORKER_USERNAME: Final[str] = "RADAR_WORKER_USERNAME"
ENV_RADAR_WORKER_PASSWORD: Final[str] = "RADAR_WORKER_PASSWORD"
...
class SimpleRadarClient(object):
...
def run_v1_radar_job(job_id: int, experiment_id: int) -> None:
    """Fetches the job files from the Radar API and runs the job.

    Args:
        job_id: The Radar job ID.
        experiment_id: The Radar experiment ID.
    """
    log = LOGGER.new(job_id=job_id, experiment_id=experiment_id)  # noqa: F841

    if (username := os.getenv(ENV_RADAR_WORKER_USERNAME)) is None:
        log.error(f"{ENV_RADAR_WORKER_USERNAME} environment variable is not set")
        raise ValueError(
            f"{ENV_RADAR_WORKER_USERNAME} environment variable is not set"
        )

    if (password := os.getenv(ENV_RADAR_WORKER_PASSWORD)) is None:
        log.error(f"{ENV_RADAR_WORKER_PASSWORD} environment variable is not set")
        raise ValueError(
            f"{ENV_RADAR_WORKER_PASSWORD} environment variable is not set"
        )

    if (api_url := os.getenv(ENV_RADAR_API)) is None:
        log.error(f"{ENV_RADAR_API} environment variable is not set")
        raise ValueError(f"{ENV_RADAR_API} environment variable is not set")

    # Instantiate a Radar client and login using worker's authentication details
    client = SimpleRadarClient(username=username, password=password, api_url=api_url)

    # Set Radar Job status to "started"
    ...
    with tempfile.TemporaryDirectory() as tempdir, set_cwd(tempdir):
        ...
        run_radar_job_path = working_dir / "run_radar_job.py"
        ...
        # Import the run_radar_job.py file as a module
        with sys_path_dirs(dirs=(str(working_dir),)):
            run_radar_job = importlib.import_module(run_radar_job_path.stem)
