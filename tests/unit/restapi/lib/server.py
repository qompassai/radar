...
class FlaskTestServer(object):
    ...
    def env(self) -> dict[str, str]:
        ...
        return (
            os.environ
            | {
                "RADAR_RESTAPI_DEV_DATABASE_URI": self._sqlite_uri,
                "RADAR_RESTAPI_ENV": "dev",
            }
            | self._extra_env
        )

    @property
    def radar_db_cmd(self) -> str:
        """The path to the radar-db command.

        Raises:
            RuntimeError: If the radar-db command is not found.
        """
        cmd = shutil.which("radar-db")

        if cmd is None:
            raise RuntimeError("radar-db command not found.")
    ...
    def upgrade_db(self, *args) -> CompletedProcess:
        ...
        return subprocess.run(
            args=[self.radar_db_cmd, "autoupgrade", *args],
            env=self.env,
            capture_output=True,
            text=True,
        )
