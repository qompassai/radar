...
PROJECT_SRC_DIR = src
PROJECT_SRC_RADAR_DIR = $(PROJECT_SRC_DIR)/radar
...
CODE_PKG_NAME = radar
CODE_BUILD_DIR = dist
CODE_COOKIECUTTER_TESTS_DIR = $(PROJECT_TESTS_DIR)/cookiecutter_radar_deployment
CODE_CONTAINER_TESTS_DIR = $(PROJECT_TESTS_DIR)/containers
CODE_INTEGRATION_TESTS_DIR = $(PROJECT_TESTS_DIR)/integration
CODE_RADAR_BUILTINS_DIR = $(PROJECT_TASK_PLUGINS_DIR)/radar_builtins
CODE_UNIT_TESTS_DIR = $(PROJECT_TESTS_DIR)/unit
CODE_SRC_FILES := $(wildcard $(PROJECT_SRC_RADAR_DIR)/generics_plugins/*.py)
CODE_SRC_FILES += $(wildcard $(PROJECT_SRC_RADAR_DIR)/generics_plugins/*/*.py)
CODE_SRC_FILES += $(wildcard $(PROJECT_SRC_RADAR_DIR)/mlflow_plugins/*.py)
CODE_SRC_FILES += $(wildcard $(PROJECT_SRC_RADAR_DIR)/pyplugs/*.py)
CODE_SRC_FILES += $(wildcard $(PROJECT_SRC_RADAR_DIR)/restapi/*.py)
CODE_SRC_FILES += $(wildcard $(PROJECT_SRC_RADAR_DIR)/restapi/*/*.py)
CODE_SRC_FILES += $(wildcard $(PROJECT_SRC_RADAR_DIR)/restapi/*/*/*.py)
CODE_SRC_FILES += $(wildcard $(PROJECT_SRC_RADAR_DIR)/rq/*.py)
CODE_SRC_FILES += $(wildcard $(PROJECT_SRC_RADAR_DIR)/rq/*/*.py)
CODE_SRC_FILES += $(wildcard $(PROJECT_SRC_RADAR_DIR)/sdk/*.py)
CODE_SRC_FILES += $(wildcard $(PROJECT_SRC_RADAR_DIR)/sdk/*/*.py)
CODE_SRC_FILES += $(wildcard $(PROJECT_SRC_RADAR_DIR)/sdk/*/*/*.py)
...
CODE_UNIT_TESTS_FILES += $(wildcard $(CODE_UNIT_TESTS_DIR)/*/*/*.py)
CODE_UNIT_TESTS_FILES += $(wildcard $(CODE_UNIT_TESTS_DIR)/*/*/*/*.py)
CODE_TASK_PLUGINS_FILES := $(wildcard $(CODE_RADAR_BUILTINS_DIR)/*.py)
CODE_TASK_PLUGINS_FILES += $(wildcard $(CODE_RADAR_BUILTINS_DIR)/*/*.py)
CODE_TASK_PLUGINS_FILES += $(wildcard $(CODE_RADAR_BUILTINS_DIR)/*/*/*.py)
CODE_TASK_PLUGINS_FILES += $(wildcard $(CODE_RADAR_BUILTINS_DIR)/*/*/*/*.py)
...
$(BEAUTIFY_SENTINEL): $(CODE_SRC_FILES) $(CODE_TASK_PLUGINS_FILES) $(CODE_UNIT_TESTS_FILES) $(CODE_CONTAINER_TESTS_FILES) $(CODE_INTEGRATION_TESTS_FILES) | $(PROJECT_BUILD_DIR)
	$(call run_black,-- $(PROJECT_SRC_RADAR_DIR) $(CODE_RADAR_BUILTINS_DIR) $(PROJECT_TESTS_DIR))
	$(call run_isort,-- $(PROJECT_SRC_RADAR_DIR) $(CODE_RADAR_BUILTINS_DIR) $(PROJECT_TESTS_DIR))
	$(call save_sentinel_file,$@)
