...
# ARG_OPTIONAL_SINGLE([gunicorn-module],[],[Python module used to start Gunicorn WSGI server],[radar.restapi.cli.gunicorn])
# ARG_OPTIONAL_REPEATED([wait-for],[],[Wait on the availability of a host and TCP port before proceeding],[])
# ARG_DEFAULTS_POS
# ARGBASH_SET_INDENT([  ])
# ARG_HELP([Radar REST API Entry Point\n])"
# ARGBASH_PREPARE
...
readonly radar_workdir="${RADAR_WORKDIR}"
...
upgrade_database() {
  echo "${logname}: INFO - Upgrading the Radar database"
