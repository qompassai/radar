...
# ARG_OPTIONAL_SINGLE([clone-branch],[],[The radar branch to clone and use for syncing task plugins],[main])
...
RADAR_GIT_REPO_URL="https://github.com/usnistgov/radar.git"
...
###########################################################################################
# Prepare a shallow clone of the radar GitHub repo
#
# Globals:
#   RADAR_GIT_REPO_URL
#   _arg_clone_branch
...
prepare_radar_repo_clone() {
  git_clone --dir-chmod "0755" --file-chmod "0644" --chown "root:root" --single-branch \
    --force --branch "${_arg_clone_branch}" "${RADAR_GIT_REPO_URL}" \
    "${_arg_init_repos_dir}"
}
