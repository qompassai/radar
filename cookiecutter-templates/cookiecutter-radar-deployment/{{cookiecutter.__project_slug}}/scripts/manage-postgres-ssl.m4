...
__enforce_ssl_connections() {
  cat <<-EOPGHBA > "${POSTGRES_PG_GBA_CONF_FILE}"
		...
		# Enforce SSL when connecting from another service as the radar user
		hostssl mlflow-tracking radar         all                     scram-sha-256
		hostssl restapi         radar         all                     scram-sha-256
	EOPGHBA
}
...
__allow_unencrypted_connections() {
  cat <<-EOPGHBA > "${POSTGRES_PG_GBA_CONF_FILE}"
		...
		host    replication     all             127.0.0.1/32            trust
		host    replication     all             ::1/128                 trust
		# Allow unencrypted connections from another service as the radar user
		host    mlflow-tracking radar         all                     scram-sha-256
		host    restapi         radar         all                     scram-sha-256
	EOPGHBA
}
